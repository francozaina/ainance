...
function App() {
  ...
  const handleCalcular = async () => {
    if (monto > 0 && plataformaSeleccionada) {
      try {
        const res = await axios.post('http://localhost:5000/api/calculos', {
          montoBruto: monto,
  ...
  // Lógica de consulta a la IA (Gemini 2.5)
  const handlePreguntarIA = async () => {
    ...
    setRespuestaIA(''); 
    try {
      const res = await axios.post('http://localhost:5000/api/chat', { mensaje: mensajeChat });
        ...
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="space-y-4">
            <div className="flex gap-3">
              ...
              >
                {cargandoIA ? 'Analizando...' : <><Send size={18} /> Preguntar</>}
              </button>
            </div>
            ...
            {respuestaIA && (
              <div className="bg-purple-50 border border-purple-100 p-5 rounded-2xl text-purple-900 animate-in fade-in zoom-in duration-300">
                <div className="flex items-start gap-3">
                  ...
                  {/* Aquí aplicamos 'prose' para que Markdown se vea con formato */}
